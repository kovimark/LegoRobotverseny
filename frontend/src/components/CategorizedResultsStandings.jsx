  ...
  return (
    <section className="card shadow-sm team-card no-hover-card mb-4">
      <div className="card-body p-4">
        ...
        <div className="d-grid gap-4">{sections.map((section) => <section key={section.key}>{showCategories && <h6 className="mb-2">{section.key === 'primary' ? 'Általános iskolás' : 'Középiskolás'}</h6>}{section.rows.length > 0 ? <><div className="scoring-table-hint d-md-none"><i className="bi bi-arrows me-1" />A teljes táblázathoz húzd oldalra.</div><div className={`table-responsive scoring-table-scroll ${tableWrapperClassName}`.trim()} tabIndex="0"><table className="table table-sm align-middle mb-0 scoring-results-table" style={{ minWidth: `${Math.max(42, 12 + columns.length * 7)}rem` }}><thead><tr><th className="text-center">#</th>{columns.map((column) => <th className={column.align === 'end' ? 'text-end' : ''} key={column.key}>{column.label}</th>)}</tr></thead><tbody>{section.rows.map((row, index) => <tr key={getKey(row, index)} className={row.isQualifier ? 'table-success' : ''}><td className="text-center fw-bold">{index + 1}</td>{columns.map((column) => <td className={column.align === 'end' ? 'text-end' : ''} key={column.key}>{column.key === 'team' && <AgeGroupBadge category={row.category} className="me-2" />}{column.render ? column.render(row, index) : row[column.key]}</td>)}</tr>)}</tbody></table></div></> : <div className="alert alert-secondary mb-0">{showCategories ? 'Ebben a korosztályban még nincs eredmény.' : (emptyText || 'Még nincs rögzített eredmény.')}</div>}</section>)}</div>
      </div>
    </section>
  )
}
