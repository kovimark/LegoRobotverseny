...
export default function UserMessagesModal({ open, onClose, user, userTeamId = null }) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [personNotifs, allMsgs] = await Promise.all([
      ...
      const combined = [...personNotifs, ...teamNotifs]
                    ...
                    <div className="card border shadow-sm rounded-3 overflow-hidden" key={notif.id || idx}>
                      ...
                      <div className="card-body py-3 px-3">
                        <p className="card-text mb-0" style={{ whiteSpace: 'pre-wrap' }}>
                          {notif.text || notif.message || 'Nincs szöveges tartalom.'}
                        </p>
                      </div>
                    </div>
                  ))
